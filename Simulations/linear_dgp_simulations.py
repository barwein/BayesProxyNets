...
if __name__ == "__main__":
    ...
    fixed_df = aux.GenerateFixedData(rng=rng, theta=THETA, eta=ETA,
                                     sig_y=SIG_Y, lin=LIN_Y, alphas=ALPHAS, n=N).get_data()
    ...
    for i in range(N_SIM):
        sim_results = one_simuation_iter(idx=i, fixed_df=fixed_df, gamma=GAMMA, gamma_rep=GAMMA_REP,
                                         eta=ETA, sig_y=SIG_Y, rho=RHO,
                                         pz=PZ, n_rep=N_REP, lin_y=LIN_Y)
