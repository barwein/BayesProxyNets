...
def one_simuation_iter(idx, fixed_df, gamma, gamma_rep, eta, sig_y, rho, pz, n_rep, lin_y):
    ...
    df_oracle = aux.SampleTreatmentsOutcomes(rng=rng, fixed_data=fixed_df, eta=eta, sig_y=sig_y, rho=rho, pz=pz, lin=lin_y).get_data()
    # Generate noisy network measurement
    obs_network = aux.create_noisy_network(rng, df_oracle["triu"], gamma, gamma_rep, df_oracle["X_diff"], df_oracle["X2_equal"])
    ...
    trills_pd_rep = pd.DataFrame({'true': df_oracle["triu"], 'obs_rep': df_obs["triu_rep"]})
    print(pd.crosstab(index=trills_pd_rep['true'], columns=trills_pd_rep['obs_rep']))

    trill_pd_obs_rep = pd.DataFrame({'obs': df_obs["triu"], 'obs_rep': df_obs["triu_rep"]})
    print(pd.crosstab(index=trill_pd_obs_rep['obs'], columns=trill_pd_obs_rep['obs_rep']))

    print("Running network module")
    # --- network module ---
    network_svi = aux.Network_SVI(data=df_obs, rng_key=rng_key, n_iter=10000, n_samples=1000, with_rep=False)
    ...
    network_svi_rep = aux.Network_SVI(data=df_obs, rng_key=rng_key, n_iter=10000, n_samples=1000, with_rep=True)
    ...
    post_zeig_rep, post_zeig_h1_rep, post_zeig_h2_rep, post_zeig_stoch1_rep, post_zeig_stoch2_rep = aux.get_post_net_stats(network_pred_samples_rep,
                                                                                                         df_obs["Z"],
                                                                                                        df_obs["Z_h"],
                                                                                                        df_obs["Z_stoch"])
    ...
    esti_post_zeig_error_rep = jnp.mean(np.abs(post_zeig_rep.mean(axis=0) - df_oracle["Zeigen"]))
    ...
    threestage_results_rep = aux.multistage_run(zeigen_post = post_zeig_rep[i_range,],
    ...
    onestage_outcome_mcmc_rep = aux.Onestage_MCMC(Y=df_obs["Y"],
                                                    X=df_obs["X"],
                                                    X2=df_obs["X2"],
                                                    Z_obs=df_obs["Z"],
                                                    Z_h=df_obs["Z_h"],
...
METHODS = ['Linear_oracle', 'BYM_oracle', 'Linear_oracle', 'BYM_oracle',
             ...
             'Linear_3S_rep','Linear_3S_rep', 'BYM_3S_rep', 'BYM_3S_rep',
             'Linear_1S', 'BYM_1S', 'Linear_1S', 'BYM_1S',
             'Linear_1S_rep', 'BYM_1S_rep', 'Linear_1S_rep', 'BYM_1S_rep']
